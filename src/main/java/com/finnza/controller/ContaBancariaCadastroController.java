...
import com.finnza.service.ContaBancariaCadastroService;
...
@RestController
@RequestMapping("/api/cadastro/contas-bancarias")
@CrossOrigin(origins = "*")
@RequiredArgsConstructor
public class ContaBancariaCadastroController {

    private final ContaBancariaCadastroService service;
    ...
    public ResponseEntity<?> listar(
            ...
            @RequestParam(value = "sort", defaultValue = "banco,asc") String sort
    ) {
        try {
            Pageable pageable = parsePageable(page, size, sort);
            Page<ContaBancariaCadastroDTO> result =
    ...
    @PostMapping
    @PreAuthorize("hasPermission(null, 'FLUXO_CAIXA')")
    public ResponseEntity<?> criar(@Valid @RequestBody ContaBancariaCadastroRequest body) {
        try {
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(service.criar(emailAutenticado(), body));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("mensagem", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> atualizar(
            ...
            @Valid @RequestBody ContaBancariaCadastroRequest body
    ) {
        try {
    ...
    private static Pageable parsePageable(int page, int size, String sort) {
        ...
        Sort srt = Sort.by(Sort.Direction.ASC, "banco");
    ...
    private static boolean isAllowedSortProperty(String prop) {
        return "banco".equals(prop)
                || "nomeConta".equals(prop)
                || "categoria".equals(prop)
                || "instituicao".equals(prop)
                || "agencia".equals(prop)
                || "conta".equals(prop)
                || "tipo".equals(prop)
